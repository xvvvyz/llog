...
export const registerActionTools = (server: McpServer, ctx: McpContext) => {
    ...
    async ({ emoji, recordId, replyId }) => {
      ...
      if (existing?.id) {
        await ctx.db.transact(
          ctx.db.tx.reactions[existing.id].update({ emoji })
        );
  ...
  server.registerTool(
    'set_record_pin',
    {
      description: 'Pin or unpin a record.',
      inputSchema: { isPinned: z.boolean(), recordId: z.string().min(1) },
    },
    async ({ isPinned, recordId }) => {
      const { record, viewer } = await getVisibleRecord(ctx, recordId);
