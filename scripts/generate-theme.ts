...
function generateNavigationTS() {
  const lightBg = oklchToHSLStr(COLORS.light.ui.background);
  const darkBg = oklchToHSLStr(COLORS.dark.ui.background);
  return `// generated by @/scripts/generate-theme.ts\n\nimport { DarkTheme, DefaultTheme } from '@react-navigation/native';\n\nexport const NAVIGATION=Object.freeze({\nlight:{...DefaultTheme,colors:{...DefaultTheme.colors,background:'hsl(${lightBg})',card:'hsl(${lightBg})'}},\n\ndark:{...DarkTheme,colors:{...DarkTheme.colors,background:'hsl(${darkBg})',card:'hsl(${darkBg})'}}\n});`;
}
