// used by @/scripts/generate-theme.ts to generate other theme files

export const COLORS = Object.freeze({
  light: {
    ui: {
      background: 'oklch(0.94 0 0)',
      foreground: 'oklch(0 0 0 / 0.95)',
      card: 'oklch(0.96 0 0)',
      cardForeground: 'oklch(0 0 0 / 0.95)',
      popover: 'oklch(1 0 0)',
      popoverForeground: 'oklch(0 0 0 / 0.8)',
      primary: 'oklch(0.6 0.2 256)',
      primaryForeground: 'oklch(1 0 0 / 0.95)',
      secondary: 'oklch(0 0 0 / 0.1)',
      secondaryForeground: 'oklch(0 0 0 / 0.8)',
      muted: 'oklch(0.96 0 0)',
      mutedForeground: 'oklch(0 0 0 / 0.65)',
      accent: 'oklch(0 0 0 / 0.05)',
      accentForeground: 'oklch(0 0 0 / 0.8)',
      destructive: 'oklch(0.6 0.2 30)',
      destructiveForeground: 'oklch(1 0 0 / 0.95)',
      border: 'oklch(0 0 0 / 0.05)',
      input: 'oklch(0 0 0 / 0.05)',
      placeholder: 'oklch(0 0 0 / 0.5)',
      ring: 'oklch(0.6 0.2 256)',
    },
    spectrum: {
      gray: 'oklch(0.65 0 0)',
      brown: 'oklch(0.65 0.04 45)',
      amber: 'oklch(0.65 0.1 60)',
      yellow: 'oklch(0.65 0.1 90)',
      green: 'oklch(0.65 0.1 144)',
      teal: 'oklch(0.65 0.1 180)',
      blue: 'oklch(0.65 0.1 216)',
      indigo: 'oklch(0.65 0.1 252)',
      purple: 'oklch(0.65 0.1 288)',
      pink: 'oklch(0.65 0.1 324)',
      red: 'oklch(0.65 0.1 0)',
      orange: 'oklch(0.65 0.1 30)',
    },
  },
  dark: {
    ui: {
      background: 'oklch(0.16 0 0)',
      foreground: 'oklch(1 0 0 / 0.95)',
      card: 'oklch(0.2 0 0)',
      cardForeground: 'oklch(1 0 0 / 0.95)',
      popover: 'oklch(0.26 0 0)',
      popoverForeground: 'oklch(1 0 0 / 0.8)',
      primary: 'oklch(0.6 0.2 256)',
      primaryForeground: 'oklch(1 0 0 / 0.95)',
      secondary: 'oklch(1 0 0 / 0.1)',
      secondaryForeground: 'oklch(1 0 0 / 0.8)',
      muted: 'oklch(0.18 0 0)',
      mutedForeground: 'oklch(1 0 0 / 0.65)',
      accent: 'oklch(1 0 0 / 0.05)',
      accentForeground: 'oklch(1 0 0 / 0.8)',
      destructive: 'oklch(0.6 0.2 30)',
      destructiveForeground: 'oklch(1 0 0 / 0.95)',
      border: 'oklch(1 0 0 / 0.05)',
      input: 'oklch(1 0 0 / 0.05)',
      placeholder: 'oklch(1 0 0 / 0.45)',
      ring: 'oklch(0.6 0.2 256)',
    },
    spectrum: {
      gray: 'oklch(0.6 0 0)',
      brown: 'oklch(0.6 0.04 45)',
      amber: 'oklch(0.6 0.1 60)',
      yellow: 'oklch(0.6 0.1 90)',
      green: 'oklch(0.6 0.1 144)',
      teal: 'oklch(0.6 0.1 180)',
      blue: 'oklch(0.6 0.1 216)',
      indigo: 'oklch(0.6 0.1 252)',
      purple: 'oklch(0.6 0.1 288)',
      pink: 'oklch(0.6 0.1 324)',
      red: 'oklch(0.6 0.1 0)',
      orange: 'oklch(0.6 0.1 30)',
    },
  },
});
